...
| 商品管理 > 店舗受取 | 店舗受取 | /admin/local_pickup_product_variants | 店舗受取対象SKUの追加・削除管理 | - |
...
| 注文管理 | 注文管理 | /admin/orders | 注文の一覧・追跡。検証注文でデータあり列と注文詳細を確認済み | 注文管理.md |
| 注文管理 > 下書き | 下書き | /admin/draft_orders | 管理画面から手動起票する注文の管理 | 注文管理.md |
| 注文管理 > 返品 | 返品 | /admin/order_returns | 返品処理の一覧確認。受領・完了は注文詳細内で扱う | 注文管理.md |
| 顧客管理 | 顧客管理 | /admin/purchasing_customers | 購入顧客の一覧・絞り込み。検証注文で直近注文/購買サマリー反映を確認済み | 顧客管理.md |
...
| 会計 | 売上実績 | /admin/sale_change_line_items | 売上実績の一覧確認・CSVエクスポート。注文で正明細、返品完了で負明細を確認済み | 会計（売上実績）.md |
...
| ディスカウント > 顧客管理 | 顧客管理 | /admin/order_price_adjustment_rules/{id}/customers | 対象顧客をメール検索で追加するタブ。同一テナントの購入顧客は追加・削除可。顧客0件またはテナント不一致では候補0件 | ディスカウント.md |
...
| OmnibusCore | OmnibusCore連携 | /admin/omnibus_core_integrations | 商品同期設定・在庫設定・注文設定を持つOmnibusCore連携設定 | OmnibusCore連携.md |
...
| 設定 > 管理メンバー（インポート） | 管理ユーザーをCSVでインポートする | /admin/csv_import/csv_import_operation_users | 管理メンバー一覧の「インポート」から遷移する管理ユーザーCSVインポート | 設定.md |
...
| 設定 > アプリ > アプリ作成 | 新しいアプリ | /admin/settings/apps/create | アプリ名と76権限スコープを指定してAPIアプリを作成 | 23-API・Webhook・開発者連携.md |
| 設定 > アプリ > アプリ詳細 | アプリ詳細 | /admin/settings/apps/{id} | Admin APIトークン/シークレット、Playground、リクエストログ、Storefront API、Webhookの導線 | 23-API・Webhook・開発者連携.md |
