...
| 店舗受取対象SKU | 商品管理 > 店舗受取 | BOPIS対象のSKUを指定 | 店舗受取可能商品 | 商品側の対象SKUと、店舗側の受取店舗設定の両方が必要 |
...
| データ | 作成・管理場所 | 主な役割 | 参照先・効く場所 | W1での扱い |
|:--|:--|:--|:--|:--|
| メタフィールド定義 | 設定 > メタフィールド定義 | 標準項目にない独自項目を追加 | 商品、バリエーション、顧客、注文、ロケーション、伝票など16オブジェクト | 定義がないと詳細画面に入力欄が出ない |
...
| 販売上限ルール | 販売設定 > 販売上限 | チャネルごとの販売数上限 | 接続済み販売チャネル | チャネル接続前は作成完了できない |
...
| 9 | メタフィールド定義 | 独自項目を使うなら必要 | 対象詳細画面に入力欄を出すため | 商品、顧客、注文、伝票等の詳細画面 |
