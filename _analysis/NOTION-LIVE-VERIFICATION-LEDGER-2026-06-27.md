...
| 08 | カスタムデータ | 5 | 22 | 13 | 中 | 2026-07-01再確認済み。メタフィールド定義作成、3文字namespace/key保存、商品詳細値保存、使用箇所0→1、削除後の一覧/商品詳細表示消滅を確認。下書き注文定義は作成・一覧表示・削除まで確認済みだが、下書き注文本体の作成/詳細画面が開けないため値入力は未確認。外部連携反映は連携待ち |
...
| 11 | 価格・販売制御 | 18 | 77 | 19 | 高 | 販売価格ルール作成/通常価格登録・削除、通常価格登録後も商品詳細価格/SKU詳細上代が変わらないこと、販売上限のShopifyチャネル候補・SKU別上限追加/削除・ルール削除まで実機確認済み。チャネル適用は連携待ち |
| 12 | 在庫状態・在庫数 | 6 | 40 | 14 | 高 | ロケーション説明、調整伝票/取置伝票後の在庫数変化を修正済み |
| 13 | 在庫伝票 | 10 | 49 | 11 | 高 | ロケーション説明、分類番号参照、調整理由4択、調整/取置伝票ステータス遷移を修正済み |
| 14 | 入出荷・在庫依頼 | 5 | 49 | 16 | 高 | 在庫依頼→移動→出荷→入荷を `#IM-1025/#IO-1025/#II-1025` で実機確認済み。2026-07-01に出荷一覧一括操作 `アクション > 納品書（PDF）`、PDF履歴失敗、入荷側のPDF/印刷/メール導線なしも確認済み |
| 15 | 発注・仕入 | 8 | 41 | 27 | 高 | 発注伝票の下書き→発注済み→入荷指示作成→入荷完了→在庫反映、別伝票のキャンセル、完了後キャンセル不可を確認済み。2026-07-01に `#IP-1014/#IP-1007` でPDF/印刷/発注確認メール手動送信/編集導線なしも確認済み。権限差分は未確認 |
...
| 20 | 標準販売チャネル連携 | 11 | 70 | 35 | 高 | 2026-06-29にShopify接続済みストア `stack-ps-yosuke` を実機確認済み。詳細5タブ、ロケーション接続、Shopify側 `erp-stg` / `SQ (エス・キュー)` 表示、商品/在庫一括同期の認証情報未設定エラーまで確認。Shopify→SQの注文・顧客・返金・キャンセル受信は確認済み。2026-06-30再確認でも連携済みバリエーション0件のため、SQ→Shopify送信と正式連携SKUでの注文在庫引当・出荷/返品は認証情報待ち/再検証 |
| 21 | 物流・返品・外部アプリ連携 | 11 | 68 | 39 | 高 | 出荷管理/Yamato B2/DHL/返品一覧/ロジザード/Recustomerを2026-06-27再確認。CSV実取込・エクスポートON実行・外部接続後挙動は連携待ち |
| 22 | CSV・PDF・データ移行 | 34 | 59 | 6 | 高 | CSVインポート/エクスポート/PDF/Yamato B2の主要導線を2026-06-27再確認。原価の所属グループとインポート完了後disabled状態を修正済み。2026-07-01に在庫CSV、ロケーションCSV、ディスカウント利用履歴CSV、セール価格CSV、商品バリエーションCSV、ポイント一括加算/減算CSV、ポイント変動履歴CSVの実データ行を確認済み |
| 23 | API・Webhook・開発者連携 | 4 | 19 | 29 | 中 | 2026-07-01再確認済み。アプリ一覧/作成/詳細、空保存エラー、Webhook作成・削除、Admin/StorefrontリクエストログTODOを確認。秘密値は証跡内でマスク済み。Storefrontトークン発行はカタログ/ロケーショングループ選択後も未表示tenantID必須で通信せず、発行完了未確認 |
| 24 | 会計・売上実績・分析 | 8 | 17 | 26 | 中 | 2026-07-01再確認済み。売上実績一覧、CSVエクスポート注文軸/明細軸の履歴列・成功完了・ダウンロードリンク、作成フォーム空保存エラー、手動作成URL404、分析3画面TODOを確認。内部GraphQL注文/返品では正負売上明細が生成される。Shopify流入注文 `#1001`〜`#1007` は注文管理には表示されるが、未連携SKUのため売上実績一覧と6/29範囲CSVには出力されない。正式連携SKUでの反映は再検証待ち |
...
### L-010B 2026-06-30 移動伝票直接作成から出荷・入荷までの在庫再チェック

- 対象: `12-在庫状態・在庫数`
- URL:
  - `/admin/inventory_movement_orders/create`
  - `/admin/inventory_movement_orders/a816b720-95a6-5a7a-87e7-c6b20c592208_InventoryMovementOrder`
  - `/admin/inventory_outbound_orders/ce669d15-cdeb-50f1-bc20-44fbb4b815f1_InventoryOutboundOrder`
  - `/admin/inventory_inbound_orders/3e3b0342-5d78-53a9-b0b8-0222c94bda9c_InventoryInboundOrder`
- TESTデータ:
  - 移動伝票: `#IM-1033`
  - 出荷指示: `#IO-1041`
  - 入荷指示: `#II-1035`
  ...
  - 数量: `1`
- 作成前在庫:
  - 移動元: 販売可能1 / 引当済み0 / 取置中0 / 手持ち1
  - 移動先: 販売可能11 / 引当済み0 / 取置中0 / 手持ち11
- 移動伝票作成:
  - 配送元・配送先を選択すると商品参照ボタンが有効化される
  - 商品選択ダイアログでは対象SKUの在庫数が `1個`
  - 商品追加後の明細数量初期値は `0`。数量を `1` に変更して保存
  - 保存後: `#IM-1033` 詳細上部は `情報 未完了 / 出荷待ち`
  - 関連として `#IO-1041` / `#II-1035` が自動生成
  - 在庫変化: 移動元は販売可能1→0、取置中0→1、手持ち1のまま。移動先は一覧に `情報 入荷予定` バッジ、主要4列は11/0/0/11のまま
- 出荷実績登録:
  - `#IO-1041` 実行前: `情報 未完了 / 出荷待ち`、引当ステータス `情報 未完了 / 引当待ち`、出荷予定1 / 出荷済み0
  - ダイアログ項目は `配送キャリア`, `追跡コード`, `キャンセル`, `登録する`
  - 配送キャリア・追跡コードは空欄のまま登録できた
  - 登録後: 上部は `成功 完了 / 出荷完了`、出荷予定1 / 出荷済み1
  - 登録後も同じ詳細画面内に `情報 未完了 / 引当待ち` が残る
  - 在庫変化: 移動元は販売可能0 / 引当済み0 / 取置中1→0 / 手持ち1→0。移動先は `情報 積送中` バッジ、主要4列は11/0/0/11のまま
- 入荷実績登録:
  - `#II-1035` 実行前: `情報 / 入荷待ち`、入荷予定1 / 入荷済み0
  - `入荷実績を登録する` はリンクで `/receive` へ遷移
  - 受領数 `今回受領` の初期値は `0`。`1` に変更して登録
  - 登録後: `#II-1035` は `成功 完了 / 入荷完了`、入荷予定1 / 入荷済み1
  - 親 `#IM-1033` も `成功 完了 / 入荷完了`
  - 最終在庫: 移動元は販売可能0 / 引当済み0 / 取置中0 / 手持ち0。移動先は販売可能11→12 / 引当済み0 / 取置中0 / 手持ち11→12
- 反映:
  - `04-notion/12-在庫状態・在庫数.md`
  - `_analysis/live-notion-verification-2026-06-30/inventory-movement-direct-recheck-20260630.md`
...
- 通常価格登録:
  ...
  - 2026-07-01追加確認: `TEST_FAQ_20260701_PRICE_NO_SKU_MUTATION_2155` で同SKUに `￥1,234` / `￥1,357` を登録しても、商品詳細のバリエーション価格は `￥1,990`、SKU詳細の `上代` は `1990` のまま。販売価格ルールの価格は商品マスタ上代を上書きしない。検証用の通常価格行とルール本体は削除済み
- 通常価格削除:
...
- ディスカウント `/admin/order_price_adjustment_rules`:
  ...
  - 2026-07-01追加確認で、必須項目を満たした新規作成は成功。`TEST_FAQ_20260701_2220_DISCOUNT_VALID_CREATE` は保存後に詳細画面へ遷移し、`成功 / 有効`、タブ `基本情報 / 利用履歴 / 顧客管理 / 店舗管理 / 商品管理`、右サイドバー `ユニクロ / 対象顧客0人 / 対象店舗0件 / 利用回数0回` を表示。
...
- 注文ポイント `/admin/point_calculation_rules`:
  ...
  - 2026-07-01追加確認で、既存 `TEST_FAQ_注文ポイント付与ルール` は `100円ごとに1ポイント`、オンライン/店舗とも付与遅延0日、会員ランク算出ルール `TEST_FAQ_会員ランク算出ルール` 紐付け済み。現時点の `会員ランク倍率` / `商品倍率` タブは空。
  - 2026-07-01追加確認で、支払い済み・発送済み内部GraphQL注文 `FAQ-ORDER-DECREMENT-SHIP-202607010132` は注文詳細に `ポイント: 付与対象に含む / ポイント付与 0ポイント`、`会員ランク: 算出対象に含む` を表示。対象顧客のポイント履歴はCSV一括加算 `+5` とCSV一括減算 `-2` のみで、注文由来行は表示されない。
- ポイントキャンペーン:
...
- ステータス/移動確認:
  - 2026-07-01追加確認で、有効期間中の削除はdisabled、説明文 `利用可能期間中、または利用履歴があるディスカウントは削除できません。` を確認。開始日時を未来に変更して保存・再読み込みすると `情報 / スケジュール済み` になり、削除可能。確認ダイアログ `ディスカウントを削除する` から削除すると一覧へ戻り、直URLは `該当するOrderPriceAdjustmentRuleが見つかりませんでした。`。
  - 注文ポイント倍率・ポイントキャンペーン条件・会員ランク反映は、正式連携SKU注文での実付与/実計算は未確認。内部GraphQL注文では対象表示に入るがポイント付与は0。
- 主なスナップショット:
...
- ヤマトB2クラウド出荷実績インポート:
  ...
  - 2026-07-01追加確認で、内部GraphQL注文から出荷待ち `#IO-1048` を作成し、`お客様管理番号=IO-1048` のCSVをDropZone経由でアップロード。詳細作成後、画面上は「CSVのバリデーションを実行しています」のままだが、内部GraphQLでは `isFailedValidationJob=true`、検証成功0件/検証失敗0件。2026-06-28の `#IO-1032` 履歴も同じ内部失敗状態。実行・出荷完了遷移は未確認。検証用 `#IO-1048` は手動出荷完了で後処理済み。
- DHL出荷実績インポート:
...
- 納品書PDFエクスポート:
  - 2026-07-01追加確認で `pdfExportOperationPackageSlipExecute` の入力は `inventoryOutboundOrderIDs` のみと確認。
  - `#IO-1048`（配送・引当待ち）と `#IO-1020`（配送・引当済み・数量5）でPDFジョブを追加作成したが、約20秒後にいずれも `isFailedExecutionJob=true`、`isCompletedExecutionJob=false`、`downloadURL=null`、`filepath=""`。
  - 既存の `#IO-1046`（配送・引当待ち）と `#IO-1047`（店頭販売による商品の受け渡し・引当待ち）も同じ失敗状態。公開GraphQL型 `PDFExportOperationPackageSlip` には失敗理由フィールドがない。
  - 納品書テンプレート `pdfTemplatePackageSlipGet` は取得でき、HTMLテンプレートは存在する（5933文字）。今回の範囲ではPDF生成成功条件は特定できず、実PDF内容・メール通知は未確認。
- 返品:
...
- ステータス・影響確認:
  - ヤマトB2出荷実績CSVは保存・詳細作成までは行ったが、内部バリデーションジョブ失敗のためCSV実行はできていない。CSVによる出荷指示の実ステータス移動・メールリンク・外部同期・返品反映は未確認。
  - 納品書PDFは出荷一覧一括アクション/GraphQL mutationでジョブ作成まではできるが、確認4件は全て失敗。引当済みでも失敗したため、引当ステータスだけを原因として案内しない。
...
- メタフィールド定義:
  ...
  - 2026-07-01追加確認: 下書き注文メタフィールド定義 `TEST_FAQ_20260701_DRAFT_ORDER_META` を `faqmeta.draft_order_20260701` / `trueまたはfalse` で保存できた。保存後は `/admin/settings/metafield_definitions?ownerType=draft_order` へ戻り、行は `編集可` / `0個の下書き注文`。行選択で `定義を削除` が表示され、確認後に削除でき、一覧は空状態に戻った。下書き注文一覧は空、`注文を作成する` は遷移せず、`/admin/draft_orders/create` は予期せぬエラーのため値入力画面は確認できない。
- 翻訳:
...
- スナップショット/結果:
  ...
  - `_analysis/live-notion-verification-2026-07-01/custom-data-draft-order-metafield-20260701.md`
...
- Webhook:
  ...
  - 2026-06-28追加確認でWebhook実作成済み。2026-07-01再確認では、作成済み行のゴミ箱アイコンから `Webhookを削除する` ダイアログが開き、本文は `Webhookを削除します。この処理は巻き戻すことができません。`。`削除する` 後はトースト `Webhookを削除しました` が出て行が消えた。
  - Webhookの編集/停止導線・行チェックボックス・送信成功/失敗ステータスは画面上に表示なし。
- リクエストログ:
  - `リクエストログを見る` は `/admin/settings/apps/{id}/admin_api` へ遷移。
  - 画面タイトルは `リクエストログ`、補足は `Admin API - TEST_FAQ_20260624_APP_113636`。
  - `/admin/settings/apps/{id}/storefront_api` も画面タイトルは `リクエストログ`、補足は `Storefront API - TEST_FAQ_20260624_APP_113636`。
  - いずれも本文は `TODO`。ログ行・ステータス・保持期間・詳細項目は確認できない。
- Storefront API:
  - `トークンを発行する` で `ストアフロント用のトークンを発行する` モーダルが開く。
  - 項目は `カタログ` と `ロケーショングループ`。空保存では `カタログを選択してください` / `ロケーショングループを選択してください`。
  - `UNIQLO` と `ユニクログループ` を選択後に `発行する` を押しても通信は発生せず、モーダルが閉じない。画面に出ない `tenantID` 必須バリデーションが残っているため、現行UIでの発行完了は未確認。
- 残件:
  - API直接実行、GraphQLスキーマ、認証ヘッダ形式、Webhook署名方式、Storefront APIの範囲は開発元確認/実行環境待ち。
  - Storefrontトークン発行後の再表示・削除・失効は未確認。
- スナップショット/結果:
  ...
  - `_analysis/live-notion-verification-2026-07-01/api-webhook-ui-20260701.md`
...
### L-030 Shopify接続済みストアの深掘り検証

- 対象: `04-notion/20-標準販売チャネル連携.md`
- 確認日: 2026-06-29
- 確認結果:
  - SQ側詳細5タブ（基本設定/リスティング/データ連携/ロケーション/アクセストークン）の現在値を再確認。
  - `データ連携` の6チェックは検証後すべてONへ復旧済み。
  - Shopify→SQの注文作成・顧客作成/更新・返金・キャンセルは、SQアクセストークン9種が未設定でも実データ反映を確認済み。
  - 6チェックOFF検証では、注文OFF・顧客OFF・キャンセルOFFは反映抑止を確認。返金OFFはOFFでもSQ注文が `返金済み` へ更新されたため、抑止として断定しない。
  - 出荷OFF/返品OFFはSQ未反映を確認したが、正式連携SKUなしではON時も未反映だったため、注文在庫引当を含めて正式連携SKUで再検証が必要。
  - リスティングはカタログ `UNIQLO`、未連携5商品、連携済み商品0件、連携済みバリエーション0件。商品/在庫一括同期は認証情報未設定で停止し、Shopify側へ部分作成なし。
  - ロケーションは `ユニクロEC`（SQ）→ `My Custom Location`（Shopify）、状態 `成功`、出荷業務 `可能`。
  - アクセストークンは `商品` / `在庫（絶対値）` / `在庫（差分値）` / `翻訳` / `注文` / `出荷指示` / `ディスカウント` / `マーケット` / `リカバリー操作` の9種すべて未設定。設定ダイアログは各種 `クライアントID` / `シークレット` 入力形式。
  - Shopify側ではインストール済みアプリ `erp-stg`、埋め込みアプリ `SQ (エス・キュー)`、連携対象 `商品データ` / `価格データ` / `在庫データ` / `注文データ` / `顧客データ`、カタログ `UNIQLO - 5点` を確認。
  - Shopify Flowは未インストールで、トリガー/アクションの実行は未検証。
- 反映:
  - `04-notion/20-標準販売チャネル連携.md`
  - `01-by-feature/Shopify連携.md`
  - `02-by-task/販売チャネルを接続する.md`
  - `03-faq/外部連携のよくある質問.md`
  - `_analysis/shopify-live-verification-2026-06-29/`

### L-031 在庫・注文・Shopify追加実機検証

- 対象: `12-在庫状態・在庫数`, `13-在庫伝票`, `14-入出荷・在庫依頼`, `16-注文・返品`, `20-標準販売チャネル連携`, 機能別記事
- 確認日: 2026-07-01
- 証跡:
  - `_analysis/live-notion-verification-2026-07-01/inventory-all-flows-20260701.md`
- 確認結果:
  - 移動伝票は、出荷前キャンセル `#IM-1034`、出荷前入荷 `#IM-1035`、部分/過入荷/手動完了 `#IM-1036`、同一ロケーション移動 `#IM-1037` を実機確認した。
  - 調整伝票 `#IA-1014` は保存直後に在庫不変、実行後に対象SKUが17/0/0/17→18/0/0/18。
  - 取置伝票 `#IR-1013` は保存直後に18/0/0/18→17/0/1/18、処理済み後に18/0/0/18へ戻った。今回の自動ブラウザではボタンクリックが通信を発火しない場面があり、画面JSと同じ完了mutationで在庫解放を確認した。
  - 在庫振替は数量欄なし。`486125-09-XL`→`486125-09-L` で選択ロケーション在庫6が全量移り、逆向きに戻した。
  - 内部GraphQLのDECREMENT注文 `FAQ-ORDER-DECREMENT-SHIP-202607010132` / `#IO-1046` は、注文作成後に18/0/0/18→17/1/0/18、出荷完了後に約8秒で17/0/0/17へ変化した。
  - Shopifyは2026-07-01時点でも連携済み商品数0、連携済みバリエーション0件、アクセストークン9種未設定。未連携商品 `バギーカーブジーンズ` 1件だけを選択して `商品情報を連携する` → `作成する` を実行したが、`ShopifyIntegrationSetProducts` は `ProductSet 用の認証情報が設定されていません` で停止。SQ側件数は0/5のまま、Shopify商品管理検索でも同商品は未作成。Shopify注文でSQ既存SKU在庫引当を検証するには、商品アクセストークン設定後に正式リスティング連携済みSKUが必要。
- 反映:
  - `04-notion/12-在庫状態・在庫数.md`
  - `04-notion/13-在庫伝票.md`
  - `04-notion/14-入出荷・在庫依頼.md`
  - `04-notion/16-注文・返品.md`
  - `04-notion/20-標準販売チャネル連携.md`
  - `01-by-feature/在庫管理.md`
  - `01-by-feature/移動伝票.md`
  - `01-by-feature/調整伝票.md`
  - `01-by-feature/取置伝票.md`
  - `01-by-feature/注文管理.md`
  - `01-by-feature/Shopify連携.md`

### L-032 販売上限ルールのShopify接続後UI検証

- 対象: `11-価格・販売制御`, `01-by-feature/販売設定`, `02-by-task/販売上限を設定する`
- 確認日: 2026-07-01
- 証跡:
  - `_analysis/live-notion-verification-2026-07-01/inventory-all-flows-20260701.md`
- 確認結果:
  - Shopify接続済み状態では、販売上限ルール作成フォームのチャネル候補に `FAQ_INTERNAL_GRAPHQL_ORDER_SHIP_FLOW` と `stack-ps-yosuke.myshopify.com` が表示された。
  - `TEST_FAQ_20260701_SALE_LIMIT_SHOPIFY` を `stack-ps-yosuke.myshopify.com` で作成でき、保存後は販売上限一覧へ戻った。
  - 詳細画面の `販売上限を追加する` から、`TEST_E2E_20260622_GU_1905_NAVY_M` を販売上限2 / 開始日時 `2026-07-01 12:00` で保存できた。
  - SKU別行は `販売上限を削除`、ルール本体は一覧行選択後の `削除する` で削除でき、検証用データは削除済み。
  - チャネル側で実際に販売数量が制限されるかは、正式連携SKUでの注文検証が必要。
- 反映:
  - `04-notion/11-価格・販売制御.md`
  - `01-by-feature/販売設定.md`
  - `02-by-task/販売上限を設定する.md`

### L-033 顧客・会社 / CRM追加実機検証

- 対象: `17-顧客・会社`, `18-CRM`, 機能別/作業別記事
- 確認日: 2026-07-01
- 証跡:
  - `_analysis/live-notion-verification-2026-07-01/customer-company-crm-20260701.md`
- 確認結果:
  - Shopify接続済み状態のポイントキャンペーン作成フォームでは、種別選択後のチャネル候補に `FAQ_INTERNAL_GRAPHQL_ORDER_SHIP_FLOW` と `stack-ps-yosuke.myshopify.com` が表示された。
  - `stack-ps-yosuke.myshopify.com` を選択した検証用ポイントキャンペーンを作成すると、詳細画面に `チャネル stack-ps-yosuke.myshopify.com` と表示された。行選択後の `ルールを削除する` から検証用キャンペーンは削除済み。
  - 内部GraphQL `orderCreate` の `OrderCreatePurchasingCompanyInput` は `companyID` / `companyLocationID` 必須、`companyContactID` 任意。検証注文 `FAQ-COMPANY-ORDER-202607011221` では注文GraphQL上の `purchasingCompany.company` / `purchasingCompany.location` に保存された。
  - ただし会社詳細/会社ロケーション詳細UIは注文・売上取得GraphQLを投げておらず、検証注文作成後も「直近の注文」は空のまま、会社詳細ロケーション行の `売上` / `注文` も `¥0` / `0` のままだった。
  - 注文詳細の配送先住所/請求先住所には住所の `company` 値が表示された。配送ラベル/PDF印字は未確認。
- 反映:
  - `04-notion/17-顧客・会社.md`
  - `04-notion/18-CRM.md`
  - `01-by-feature/会社.md`
  - `01-by-feature/ポイント.md`
  - `02-by-task/ポイントキャンペーンを設定する.md`

### L-034 商品バリエーションCSVエクスポート実ファイル差分検証

- 対象: `22-CSV・PDF・データ移行`, CSVエクスポート機能別/作業別/FAQ
- 確認日: 2026-07-01
- 証跡:
  - `_analysis/live-notion-verification-2026-07-01/csv-export-product-variants-20260701.md`
- 確認結果:
  - `/admin/csv_export/csv_export_operation_product_variants/create` で `商品情報を含める` OFF/ONをそれぞれ実行した。
  - OFF行は履歴一覧で `成功 完了` / `含めない`、ON行は `成功 完了` / `成功 含める` と表示され、どちらも `ダウンロード` リンクが出た。
  - OFFのCSVは `SKU,ProductVariantExternalID,ProductVariantTitle...` から始まるバリエーション列のみ。
  - ONのCSVは先頭に `ProductCode`, `ProductExternalID`, `ProductTitle`, `ProductStatus`, `ProductType`, `ProductVendor`, `BrandName`, `BrandCode` が追加された。
  - 検証ファイルはいずれも98行。ONは商品本体列が増えるためファイルサイズが 8,344 bytes から 14,228 bytes に増えた。
- 反映:
...
### L-035 在庫CSVエクスポート実ファイル検証

- 対象: `22-CSV・PDF・データ移行`, CSVエクスポート機能別/作業別/FAQ
- 確認日: 2026-07-01
- 証跡:
  - `_analysis/live-notion-verification-2026-07-01/csv-export-inventory-20260701.md`
- 確認結果:
  - `/admin/csv_export/csv_export_operation_inventory_logical_quantities/create` のフォーム項目は `ロケーション` のみで、カタログ指定はない。
  - ロケーション選択ダイアログは `すべて / 店舗 / 倉庫` タブ、列は `名前 / 場所コード`。
  - `ユニクロ物流倉庫` を1件チェックすると、別行の `ユニクロ - 銀座店` はdisabledになり2件目をチェックできなかった。過去の「複数選択可」は修正し、実質1ロケーション指定として扱う。
  - `ユニクロ物流倉庫` で実行すると、履歴一覧に `作成日 / 店舗 / ステータス / ダウンロード` 列が表示され、開始直後は `注意 処理中`、完了後は `成功 完了` と `ダウンロード` リンクが表示された。
  - 実CSVは98行、ヘッダーは `SKU,ProductVariantID,InventoryItemID,Incoming,Available,Committed,Reserved,Damaged,SafetyStock,QualityControl,OnHand`。CSV本体にロケーション名・場所コード列はない。
- 反映:
  - `04-notion/22-CSV・PDF・データ移行.md`
  - `01-by-feature/CSVエクスポート・PDFエクスポート.md`
  - `02-by-task/データをCSV・PDFでエクスポートする.md`
...
### L-036 ロケーションCSVエクスポート実ファイル検証

- 対象: `22-CSV・PDF・データ移行`, CSVエクスポート機能別/作業別/FAQ
- 確認日: 2026-07-01
- 証跡:
  - `_analysis/live-notion-verification-2026-07-01/csv-export-locations-20260701.md`
- 確認結果:
  - `/admin/csv_export/csv_export_operation_location_by_location_group/create` のフォーム項目は `ロケーショングループ` のみ。
  - 選択ダイアログには `テスト`, `TEST_FAQ_20260624_LG_171810`, `ユニクログループ`, `GU グループ` が表示された。
  - `ユニクログループ` を1件チェックすると他グループはdisabledになり、実質1グループ指定だった。
  - 実行後の履歴一覧に `作成日 / ロケーショングループ / ステータス / ダウンロード` 列が表示され、開始直後は `注意 処理中`、完了後は `成功 完了` と `ダウンロード` リンクが表示された。
  - 実CSVは4行（ヘッダー + 3ロケーション）。ヘッダーは `Name,DisplayName,ExternalID,IsListedPublic,IsArchived,LocationCode,Tags,Email,LocationType,PointApplicationType,IsLocalPickupEnabled,IsInventoryAllocationRequestEnabled,MapURL,Phone,CountryCode,PostalCode,Province,City,Address1,Address2`。
  - `LocationType` は `RETAIL` / `WAREHOUSE`、`IsLocalPickupEnabled` と `IsInventoryAllocationRequestEnabled` も列として出力された。
- 反映:
  - `04-notion/22-CSV・PDF・データ移行.md`
  - `01-by-feature/CSVエクスポート・PDFエクスポート.md`
  - `02-by-task/データをCSV・PDFでエクスポートする.md`
...
### L-037 ディスカウント利用履歴CSVエクスポート実ファイル検証

- 対象: `22-CSV・PDF・データ移行`, CSVエクスポート機能別/作業別/FAQ
- 確認日: 2026-07-01
- 証跡:
  - `_analysis/live-notion-verification-2026-07-01/csv-export-discount-usages-20260701.md`
- 確認結果:
  - `/admin/csv_export/csv_export_operation_order_price_adjustment_usages/create` のフォーム項目は `ディスカウント` のみ。
  - 選択ダイアログは `タイトル / ステータス` 列で、有効な候補2件が表示された。
  - `TEST_FAQ_ディスカウント_対象商品テスト` を1件チェックすると他候補はdisabledになり、実質1ディスカウント指定だった。
  - 実行後の履歴一覧に `作成日 / ディスカウント / ステータス / ダウンロード` 列が表示され、開始直後は `注意 処理中`、完了後は `成功 完了` と `ダウンロード` リンクが表示された。
  - 利用履歴0件でもヘッダーのみのCSVをダウンロードできた。ヘッダーは `顧客ID,顧客外部ID,店舗名,店舗コード,利用日時`。
...
### L-038 セール価格CSVエクスポート実ファイル検証

- 対象: `22-CSV・PDF・データ移行`, CSVエクスポート機能別/作業別/FAQ
- 確認日: 2026-07-01
- 証跡:
  - `_analysis/live-notion-verification-2026-07-01/csv-export-sale-prices-20260701.md`
- 確認結果:
  - `/admin/csv_export/csv_export_operation_product_price_rule_sale_prices/create` のフォーム項目は `販売価格ルール` のみ。
  - 販売価格ルールはセレクトボックスで、候補に `年末セール価格 (JPY)` など5件が表示された。
  - `年末セール価格 (JPY)` を選択して実行すると、履歴一覧に `作成日時 / 販売価格ルール / ステータス / ダウンロード` 列が表示され、開始直後は `注意 処理中`、完了後は `成功 完了` と `ダウンロード` リンクが表示された。
  - 実CSVは2行（ヘッダー + 1件）。ヘッダーは `ProductVariantID,ExternalID,PriceAmount,PriceAmountAfterTaxes,CurrencyCode,StartsAt,EndsAt`。
  - サンプル行は `PriceAmount=1000`, `PriceAmountAfterTaxes=1100`, `CurrencyCode=JPY`, `StartsAt=2026-06-30 15:00:00`, `EndsAt=2026-07-07 14:59:00`。
- 反映:
  - `04-notion/22-CSV・PDF・データ移行.md`
  - `01-by-feature/CSVエクスポート・PDFエクスポート.md`
  - `02-by-task/データをCSV・PDFでエクスポートする.md`
...
### L-039 ポイント変動履歴CSVエクスポート実ファイル検証

- 対象: `22-CSV・PDF・データ移行`, CSVエクスポート機能別/作業別/FAQ
- 確認日: 2026-07-01
- 証跡:
  - `_analysis/live-notion-verification-2026-07-01/csv-export-point-changes-20260701.md`
- 確認結果:
  - `/admin/csv_export/csv_export_operation_point_changes/create` のフォーム項目は `テナント`, `開始日時`, `終了日時`。
  - `ユニクロ`、対象期間 `2026-06-01 00:00 〜 2026-07-01 23:59` を指定して実行した。
  - 実行後の履歴一覧に `作成日 / テナント / 対象期間 / ステータス / ダウンロード` 列が表示され、開始直後は `注意 処理中`、完了後は `成功 完了` と `ダウンロード` リンクが表示された。
  - 実績0件時の実CSVは1行（ヘッダーのみ）。ヘッダーは `PointChangeID,PurchasingCustomerID,PurchasingCustomerBarcode,Title,Delta,AvailableAt,ExpiresDate,PointChangeCreatedAt,PointChangeActivityType,Note`。
  - 同日追加でポイント一括加算・減算後に再エクスポートし、加算 `FAQ_POINT_PLUS_20260701` は `Delta=5` / `PointChangeActivityType=EARNED`、減算 `FAQ_POINT_MINUS_20260701` は `Delta=-2` / `PointChangeActivityType=USED` と出力されることを確認した。
- 反映:
  - `04-notion/22-CSV・PDF・データ移行.md`
  - `01-by-feature/CSVエクスポート・PDFエクスポート.md`
  - `02-by-task/データをCSV・PDFでエクスポートする.md`
...
### L-040 ポイント一括加算・減算CSV 実データ取込検証

- 対象: `22-CSV・PDF・データ移行`, CSVインポート機能別/作業別/FAQ
- 確認日: 2026-07-01
- 証跡:
  - `_analysis/live-notion-verification-2026-07-01/point-csv-import-plus-minus-20260701.md`
  - `_analysis/live-notion-verification-2026-07-01/point-plus-import-20260701.csv`
  - `_analysis/live-notion-verification-2026-07-01/point-minus-import-20260701.csv`
  - `_analysis/live-notion-verification-2026-07-01/point-change-after-minus-20260701-2119.csv`
- 確認結果:
  - 一括加算フォームは `テナント`, `顧客IDの種別`, CSVファイル。`顧客IDの種別` は `SQが発番した顧客ID` / `外部システムが発番した顧客ID`。
  - 一括加算CSVヘッダーは `customer_id,title,points,available_date,expires_date,note`。`ユニクロ`、SQ顧客ID、`FAQ_POINT_PLUS_20260701` 5ポイントで検証成功1件・実行成功1件。
  - 一括減算フォームは `テナント` とCSVファイルのみで、顧客IDの種別フィールドはない。
  - 一括減算CSVヘッダーは `customer_id,title,points,note`。SQ顧客IDでは `customer_id: 指定された顧客IDの顧客が存在しません` で検証失敗。外部ID `FAQ_E2E_UNIQLO_CUSTOMER_20260628095318` では検証成功1件・実行成功1件。
  - 顧客詳細の保有ポイントは `0 -> 5 -> 3`。履歴は加算 `5`、減算 `-2`。
  - 減算CSVの `points` は正数で入力し、顧客詳細・ポイント変動履歴CSVでは負数として反映される。
- 反映:
  - `04-notion/22-CSV・PDF・データ移行.md`
  - `01-by-feature/CSVインポート.md`
  - `02-by-task/ポイントをCSVで一括加算・減算する.md`
  - `03-faq/CSVインポート・エクスポートのよくある質問.md`
  - `_analysis/WBS-JULY-END-SCOPE-REVIEW-2026-06-21.md`

### L-041 入出荷・発注のPDF/メール/編集導線再確認

- 対象: `14-入出荷・在庫依頼`, `15-発注・仕入`, 出荷/入荷/発注機能別、CSV/PDF関連
- 確認日: 2026-07-01
- 証跡:
  - `_analysis/live-notion-verification-2026-07-01/area14-15-inbound-outbound-purchase-pdf-mail-edit-20260701.md`
- 確認結果:
  - 出荷詳細 `#IO-1047` にはPDF/印刷/メール/編集/その他操作なし。ただし出荷一覧で対象行をチェックすると一括操作 `アクション > エクスポート > 納品書（PDF）` が表示された。
  - `納品書（PDF）` は確認モーダル後に `/admin/pdf_export/pdf_export_operation_packing_slips` へ遷移し、PDF出力履歴を作成する。`#IO-1047` と通常配送 `#IO-1046` はどちらも `重大 / 失敗`、ダウンロードdisabled。
  - 入荷一覧には行チェックボックス/一括アクションなし。`#II-1038`（入荷完了）と `#II-1039`（入荷キャンセル）の詳細、および入荷管理JSにPDF/印刷/メール/エクスポート導線なし。入荷完了/入荷実績登録リンクは表示されるが disabled + hrefなし。
  - 発注済み `#IP-1014` と入荷完了済み `#IP-1007` の詳細・その他操作・発注詳細JSにPDF/印刷/発注確認メール手動送信/編集/保存/更新/差し戻し導線なし。`#IP-1014` のキャンセルは有効、`#IP-1007` は入荷完了後のためキャンセルdisabled。
- 反映:
  - `04-notion/14-入出荷・在庫依頼.md`
  - `04-notion/15-発注・仕入.md`
  - `01-by-feature/出荷管理.md`
  - `01-by-feature/入荷管理.md`
  - `01-by-feature/発注管理.md`
  - `01-by-feature/CSVエクスポート・PDFエクスポート.md`
  - `02-by-task/ヤマトB2クラウドで出荷業務を行う.md`
  - `00-getting-started/SQ完全ガイド.md`
...
3. ステータス遷移が重要な領域は、作成後のステータス移動・一覧タブ移動・在庫区分変化まで検証する。`13-在庫伝票`, `14-入出荷・在庫依頼`, `15-発注・仕入`, `16-注文・返品` は内部/管理画面で主要フロー確認済み。Shopify正式連携SKUでの注文在庫引当・出荷/返品は連携待ち。
