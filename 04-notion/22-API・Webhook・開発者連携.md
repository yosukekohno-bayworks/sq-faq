...
| 権限（チェックボックス群） | — | 76権限から付与するスコープを個別に選択。各権限には説明文が付いている（例: 「商品情報を閲覧することができます。」）**チェックを入れたリソースだけ、このアプリからAPI経由で読み書き可能になる**（例: 「在庫:read」のみなら在庫参照のみで書き込みは不可）。付与しすぎるとトークン漏洩時の被曝範囲が広がるため、用途に対して最小限のスコープを選ぶ |
...
「**Playgroundを開く**」ボタンから GraphQL Playground を開いてAdmin APIを試すことができます。アプリの権限でログイン状態になり、**認証ヘッダを意識せずクエリを試せる検証用画面**です（トークンの具体的な渡し方は実機未確認・確認中）。APIを直接呼ぶ前の最初の動作確認手段として使います。
...
> - **注文の作成**: 新規注文が発生するたびに通知（受注の即時取り込みに使う）
> - **注文の更新**: 既存注文のステータス・内容が変更されるたびに通知（出荷完了・キャンセル等の変化を拾う）
> - **在庫の更新**: 在庫数が変更されるたびに通知（在庫連動・欠品検知に使う）
>
> いずれも「発生するたびに都度通知」する挙動です。注文の作成と更新の違いは新規発生か既存変更か。在庫の更新が対象とする範囲（全ロケーションか特定か）は実機未確認です。
