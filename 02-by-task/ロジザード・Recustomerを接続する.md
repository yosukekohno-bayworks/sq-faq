...
> 対象ユーザー: 管理者・運営者　|　所要: 各連携 5〜15分（認証情報の準備済みの場合）　|　最終確認: 2026-06-28
...
<!-- 連携実接続対象外: 各連携共通: 保存後の接続確立・同期開始の挙動は未接続環境のため未確認 -->
...
3. 「アカウントを接続」ボタンをクリックする。接続フォーム（`/admin/recustomer_integrations/create`）が開く（2026-06-28再確認）。
4. 次の項目を入力する（どちらも必須）。

![Recustomerの「アカウントを接続する」フォーム](https://raw.githubusercontent.com/yosukekohno-bayworks/sq-faq/main/_analysis/screenshots/14-recustomer-create.png)
...
<!-- 連携実接続対象外: 「保存する」押下後の接続確立・連携開始の挙動は未接続環境のため未確認 -->

> **接続済みデータの削除（2026-06-24実機確認）**: Recustomer連携一覧（`/admin/recustomer_integrations`）で登録済み行のチェックボックスを選択すると、上部に `アクション` メニューが表示されます。`接続を削除する` → 確認ダイアログ `削除する` で接続を削除できます。確認ダイアログには「この処理は巻き戻すことができません」と表示されるため、誤削除に注意してください。

### うまくいかないとき（Recustomer）

| 症状 | 対処 |
|:--|:--|
| ストアIDまたはシークレットが空のまま保存しようとした | どちらも必須項目です。2026-06-28再確認では `ストアIDを入力してください` / `シークレットを入力してください` が表示されました。Recustomerの管理画面でストアIDとシークレットを確認してから再入力してください |
...
![「ロジザード連携を作成する」フォーム（全セクション）](https://raw.githubusercontent.com/yosukekohno-bayworks/sq-faq/main/_analysis/screenshots/14-logizard-create-full.png)
...
<!-- 外部出力/実行例外: 「保存する」押下後の接続確立・在庫/入荷/出荷データの同期開始の挙動は未接続環境のため未確認 -->
<!-- 連携実接続対象外: ロジザード接続済み状態の詳細・編集・解除操作画面は未確認 -->

### うまくいかないとき（ロジザード）

| 症状 | 対処 |
|:--|:--|
| 必須フィールドのエラーが複数表示される | 各セクションの必須項目（*印）がすべて入力されているか確認してください。2026-06-27再確認では、設定名、3桁のグループ番号/接続番号、オーナーID、ユーザーID、パスワード、アプリケーションキー、入荷/出荷/出荷箱明細実績エクスポートの各ファイルID・パターンIDに未入力エラーが表示されました |
...
![アプリ作成フォーム（アプリ名と権限のチェックボックス）](https://raw.githubusercontent.com/yosukekohno-bayworks/sq-faq/main/_analysis/screenshots/14-apps-create.png)
...
アプリを保存すると詳細画面が開き、以下の情報とセクションが表示されます。アクセストークン/シークレットは秘密情報のため、FAQやログに出力しないでください。2026-07-01の実機確認では、アプリ作成後の詳細画面・上部「その他の操作」・一覧カードとも、アプリ削除・トークン失効・アクセストークン/シークレット再発行導線は確認できませんでした。誤作成時の削除・失効方法は開発元確認が必要です。
...
「**Playgroundを開く**」ボタンから GraphQL Playground を開けます。2026-06-28再確認では、stagingのアプリ詳細からでも本番ドメイン `https://sq.stackservice.com/api/admin/v1/playground` が開き、URLクエリにトークン/アプリIDらしき値は付与されませんでした。実行時の認証ヘッダ・スキーマは開発元確認または許可後検証が必要です。

#### リクエストログ

「**リクエストログを見る**」からこのアプリによるAdmin APIリクエスト履歴画面へ遷移できます。2026-07-01時点では本文は `TODO` です。Storefront APIログの直URL `/admin/settings/apps/{id}/storefront_api` も `リクエストログ` 画面で本文は `TODO` でした。

#### Storefront API

「**トークンを発行する**」ボタンから Storefront API 用トークンの発行モーダルを開けます。2026-07-01実機確認では、モーダル項目は `カタログ` と `ロケーショングループ` です。空保存エラーは `カタログを選択してください` / `ロケーショングループを選択してください`。ただし選択後に `発行する` を押しても通信が発生せず、画面に出ない `tenantID` 必須バリデーションが残っているため、現行UIでの発行完了は未確認です。
...
未入力で保存すると、`イベントを選択してください` / `エンドポイントを入力してください` が表示されます。2026-07-01追加確認では、イベント `注文の作成`、検証用 `.invalid` エンドポイントでWebhookを実作成でき、保存後はWebhookセクションにイベント名・エンドポイント・形式 `JSON` が表示されました。作成済み行のゴミ箱アイコンから削除確認を開け、`削除する` 後はトースト `Webhookを削除しました` が出て行が消えます。編集/停止導線と送信成功/失敗ステータスは確認できませんでした。
