...
アプリを保存すると詳細画面が開き、以下の情報とセクションが表示されます。アクセストークン/シークレットは秘密情報のため、FAQやログに出力しないでください。2026-06-24の実機確認では、アプリ作成後の詳細画面・一覧カードとも削除導線は確認できませんでした。誤作成時の削除・失効方法は開発元確認が必要です。
...
未入力で保存すると、`イベントを選択してください` / `エンドポイントを入力してください` が表示されます。2026-06-28追加確認では、イベント `注文の作成`、検証用 `.invalid` エンドポイントでWebhookを実作成でき、保存後はWebhookセクションにイベント名・エンドポイント・形式 `JSON` が表示されました。作成直後の画面上では、Webhook単位の編集/削除/停止導線と送信成功/失敗ステータスは確認できませんでした。
